...
status display(hash_t *table)
{
    // Box top + headings in green
    printf(G "\n╔════╦════════════════════════╦════════════════════╦════════════╦════════════╗\n" Re);
    printf(G "║Idx ║ Word                   ║ File Name          ║ FileCount  ║ WordCount  ║\n" Re);
    printf(G "╠════╬════════════════════════╬════════════════════╬════════════╬════════════╣\n" Re);
    ...
    for(int i = 0; i < 28; i++)
    {
        ...
        if(table[i].mlink != NULL)
        {
            ...
            // if not empty move to each main nodes and print its data
            while(temp != NULL)
            {
                sub_t *temp1 = temp->slink;
                while(temp1 != NULL)
                {
                    // Data printed in normal color
                    printf("║%-3d ║ %-22s ║ %-18s ║ %-10d ║ %-10d ║\n",
                            i, temp->word, temp1->f_name, temp->f_count, temp1->w_count);
                    temp1 = temp1->slink;
                }
                temp = temp->mlink;
            }
